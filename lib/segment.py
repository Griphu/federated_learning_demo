# Modified version of breast_segment from https://github.com/olieidel/breast_segment
...
def segment(im, scale_factor=0.25, threshold=3900, felzenzwalb_scale=0.15):
    """
    Fully automated breast segmentation in mammographies.
    Modified version of https://github.com/olieidel/breast_segment
    ...
    left = np.array(col_sums_split[0])/10000.
    right = np.array(col_sums_split[1])/10000.

    left_col_sum = np.sum(left)
    right_col_sum = np.sum(right)
    ...
    im_small = im_thres[0::4, 0::4]
    n_min = np.min(im_small)
    n_max = np.max(im_small)
    im_small = np.interp(im_small, (n_min, n_max), (0, 1))
    ...
    left_mask = np.array(col_sums_split[0])/10000.
    right_mask = np.array(col_sums_split[1])/10000.

    left_col_sum = np.sum(left_mask)
    right_col_sum = np.sum(right_mask)
