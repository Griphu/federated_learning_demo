...
model.classifier = nn.Sequential(nn.Linear(in_features = 1280,out_features = 640),
                                 nn.ReLU(inplace = True),
                                 nn.Linear(in_features = 640,out_features = 3))
...
def load_data():
    ...
    norm = [0.5, 0.5, 0.5]
    train_transform = transforms.Compose([transforms.Resize((250, 250)),
                                          transforms.ToTensor(),
                                          transforms.Normalize(norm, norm)])

    test_transform = transforms.Compose([transforms.Resize((250, 250)),
                                         transforms.ToTensor(),
                                         transforms.Normalize(norm, norm)])
